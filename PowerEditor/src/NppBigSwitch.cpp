			...
			else
			{
				if ((HWND(wParam) == _mainEditView.getHSelf()) || (HWND(wParam) == _subEditView.getHSelf()))
				{
					...
					if (((ctrlKey & 0x8000)) && rMBtn) { //if ctrl key + right click down
						...
						//else if at center, move cursor to the center of context menu
						/*else {
							xMouseCoor = (contextMenuWidth / 2);
							yMouseCoor = (contextMenuHeight / 2);
						}*/

						//set new cursor coordinate based on cases above
...
LRESULT CALLBACK CustomContextMenuProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE: //put all sorts of stuff when the context menu is called
	{
		...
		for (int i = 0; i < menuItemNameMap.size(); i++) {
			if (counter == 0) { //set at initial spot at left
				theXCoor = initialXCoor;
				theYCoor = initialYCoor;
			}
			else if (counter == 1) { //same level as [0], but at right side
				theXCoor = xGap; //set at right
				theYCoor = initialYCoor; //same level as [0]
			}
			else if (counter % 2 == 0) { //if even go left
				theXCoor = initialXCoor; //always at left
				nextYCoor += yGap; //put under [0]
				theYCoor = nextYCoor;
			}
